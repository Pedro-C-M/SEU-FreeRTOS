...
void app_main()
{
    ...
    for (unsigned int i = 0; i < NUM_ACQ_SENSORS; i++)
    {
        ...
        char task_name[16];
        sprintf(task_name, "TaskAcq_%d", i);
        
        xTaskCreate(HTAcquisition, task_name, TASK_STACK_SIZE, &params[i], TASK_ACQ_PRIORITY, &xHandleAcq[i]);
        configASSERT( xHandleAcq[i] );
        ESP_LOGI(TAG, "[app_main] Task_Acq %d created.", i);
    }

    xTaskCreate(HTDisplay, "TaskDisp", TASK_STACK_SIZE, xHTQueue, TASK_DISP_PRIORITY, &xHandleDisp);
